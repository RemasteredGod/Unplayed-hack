  ...
  class ThreeDStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._boot().catch((err) => {
        ...
        this._err.textContent = /webgl/i.test(msg)
          ? 'WebGL is unavailable in this view.\n' +
            'The 3D model loaded, but this browser/preview could not create a ' +
            'GPU context. Open the file in Chrome or Safari with hardware ' +
            'acceleration enabled (Chrome: Settings \u2192 System \u2192 "Use ' +
    ...
    async _boot() {
      ...
      // Some sandboxed/headless views refuse a GPU context on the first,
      // fanciest set of options. Degrade step by step (own canvas + explicit
      // context that tolerates a software renderer) before giving up.
      const canvas = document.createElement('canvas');
      const ctxOpts = { alpha: true, preserveDrawingBuffer: true, failIfMajorPerformanceCaveat: false, antialias: false, depth: true, stencil: false };
      const getCtx = (kind) => { try { return canvas.getContext(kind, ctxOpts); } catch (e) { return null; } };
      const attempts = [
        { antialias: true, alpha: true, preserveDrawingBuffer: true },
        { antialias: false, alpha: true, preserveDrawingBuffer: true, powerPreference: 'default', failIfMajorPerformanceCaveat: false },
        () => { const c = getCtx('webgl2'); return c && { canvas, context: c, antialias: false, alpha: true, preserveDrawingBuffer: true }; },
        () => { const c = getCtx('webgl') || getCtx('experimental-webgl'); return c && { canvas, context: c, antialias: false, alpha: true, preserveDrawingBuffer: true }; },
      ];
      let renderer = null, lastErr = null, degraded = false;
      for (let i = 0; i < attempts.length; i++) {
        const a = attempts[i];
        const opts = typeof a === 'function' ? a() : a;
        if (!opts) continue;
        try { renderer = new THREE.WebGLRenderer(opts); degraded = i > 0; break; }
        catch (e) { lastErr = e; }
      }
      if (!renderer) throw (lastErr || new Error('Error creating WebGL context.'));
      renderer.setPixelRatio(degraded ? 1 : Math.min(window.devicePixelRatio || 1, 2));
